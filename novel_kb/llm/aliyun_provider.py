...
class AliyunProvider(LLMProvider):
    """阿里云通义千问 Provider（兼容 OpenAI API）"""
    
    def __init__(self, config) -> None:
        self.api_key = config.aliyun_api_key
        self.base_url = config.aliyun_base_url.rstrip("/")
    ...
    def _require_client(self) -> Any:
        if not self.client:
            raise ProviderError("Aliyun client is not configured")
        return self.client
