...
class OpenAILikeProvider(LLMProvider):
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Health check failed for provider=%s model=%s: %s",
                self.provider_name,
                self.model,
                exc,
            )
    ...
    def _require_client(self) -> Any:
        if not self.client:
            raise ProviderError(f"{self.provider_name} client is not configured")
        return self.client
    ...
    def _extract_text_content(self, response: Any) -> str:
        # 1) 非标准 provider 可能直接返回字符串
        if isinstance(response, str):
            return response

        # 1.5) 新版 SDK 对象可先转 dict 再走兼容解析
